...
// AVFDT: 3rd May. Adding the variable eChunk. 
...
static void _DoContinuousSplit(VFDTPtr vfdt,
   ...
   gd->parentClass = parentClass;
	...
	//AVFDT
   gd->eChunk = vfdt->processChunkSize;
...
static void _CheckSplit(VFDTPtr vfdt, DecisionTreePtr currentNode, int forceSplit) {
   ...
   ExampleGroupStatsPtr egs = gd->egs;
	//printf("echunk inside checksplit:  %d examples: %d \n", gd->eChunk, gd->seenSinceLastProcess);
   ...
   if(!gd->prePruned) {
      ...
      if((secondIndex - firstIndex > hoeffdingBound)
         ...
         (forceSplit && ExampleGroupStatsNumExamplesSeen(egs) > 5 &&
          ...
          if(vfdt->messageLevel >= 1) {
               printf("Splitting with chunk value: %d\n", gd->eChunk );
            }
      ...
      // EEFVDT: 3rd May I think nullIndex here is incorrect 
		...
		if(vfdt->messageLevel >= 1) {
        	printf("Tie splitting with chunk value: %d\n", gd->eChunk );
        }
         ...
         // Not splitting. 
         // AVFDT 3rd may: If not split, update eChunk. eChunk is going to be the n so that epsilon=deltaG 

         // AVFDT 8th May. If attributes are too similar, nmin is going to be controlled by tau 
		int tmp_chunk = gd->eChunk;
   ...
   //     (2.0 * ExampleGroupStatsNumExamplesSeen(egs)));
         ...
         if(vfdt->messageLevel >= 2) {
            printf("Not splitting. Seeting g to new chunk value: %d, initial chunk: %d , with differences between attributes: %f. Hoeffding %f \n",
                     gd->eChunk, tmp_chunk, secondIndex - firstIndex, hoeffdingBound );
            //ExampleGroupStatsWrite(egs, stdout);
         }
